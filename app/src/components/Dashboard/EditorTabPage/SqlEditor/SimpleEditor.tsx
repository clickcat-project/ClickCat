...
export interface SimpleEditorProps {
  ...
  processSql?: boolean;
  onMount?: () => void;
  onExecCommand?: (queryList: Query, isExecAll: boolean) => void;
}

loader.config({
  // https://unpkg.com/monaco-editor@0.28.1/min/vs
  // https://cdn.staticfile.org/monaco-editor/0.19.3/min/vs/
  // https://cdnjs.cloudflare.com/ajax/libs/monaco-editor/0.34.0-dev.20220530/min/vs/
  paths: {
    vs: 'https://unpkg.com/monaco-editor@0.28.1/min/vs'
  }
})
...
export default class SimpleEditor extends React.Component<SimpleEditorProps> {
   ...
   * @param isAllQuery
   */
  public onExecCommand = (isAllQuery: boolean, rows: string) => {
    let sql;
    if(!this.currentEditor!.getSelection()!.isEmpty()){
      sql = this.currentEditor!.getModel()!.getValueInRange(this.currentEditor!.getSelection()!)!.trim();
    }else{
      sql = this.currentEditor!.getValue().trim();
    }
    ...
    const query = new Query(sql, this.helper().makeQueryId());
    query.setJsonFormat();
    const { onExecCommand } = this.props;
    if (onExecCommand) {
      onExecCommand(query, isAllQuery);
    }
  };
