...
export const formatData = (data: { '0': { '0': any, '1': any } }) => {
  ...
  const diff = Object.keys(forecast).map(key => {
    return [moment(+realKey[key]).format('YYYY-MM-DD HH:mm:ss'), real[realKey[key]]]
  })
...
export function formatLineOptions(realData: number[], forecastData: number[], realKey: string[], diff: number[]): echarts.EChartsCoreOption {
  return {
    tooltip: {
      trigger: 'axis',
      axisPointer: {
