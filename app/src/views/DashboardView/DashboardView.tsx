...
class DashboardView extends React.Component<RoutedProps> {
  ...
  private getTabIcon = (tab: Tab): JSX.Element => {
    if (tab.engine?.match(/MaterializedView.*/)) return <EyeOutlined />;
  ...
  render() {
    ...
    return (
      <div>
        <Page style={{ height: '100%', paddingTop: '64px' }} column={false} uiStore={tabsStore.uiStore}>
          <Splitter
            className={css.setHeight}
            primary="second"
            minSize={100}
            maxSize={-100}
            defaultSize="calc(100vw - 290px)"
