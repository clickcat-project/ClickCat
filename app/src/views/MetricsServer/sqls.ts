...
function queryVersion() {
  return new Query(formatJson(`SELECT version()`));
}
/**
 * 查询运行时间
 */
function queryServerUptime() {
  return new Query(formatJson(`SELECT uptime() as uptime`));
}
/**
 * 查询总数据条数
 */
function queryTotalRows(database?: string, table?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
    { key: 'name', value: table },
  ]);
  return new Query(
    formatJson(`SELECT sum(total_rows) as "Total rows" FROM system.tables${whereStr}`)
  );
}
/**
 * 查询总列数
 */
function queryTotalColumns(database?: string, table?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
    { key: 'name', value: table },
  ]);
  return new Query(formatJson(`SELECT count() as "Total columns" FROM system.columns${whereStr}`));
}
/**
 * 查询硬盘利用率
 */
 ...
 * 查询表数据量排序前10
 */
function queryTopTablesByRows(database?: string, table?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
    { value: 'active' },
    { key: 'name', value: table },
  ...
  return new Query(
    formatJson(`SELECT concatAssumeInjective(table.database, '.', name) as name,
 ...
 * 查询列数据排序前10
 */
function queryTopTablesByColumns(database?: string, table?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
    { value: "name != ''" },
 ...
 * 查询数据库引擎
 */
function queryDatabaseEngines(database?: string) {
 ...
 * 查询表引擎
 */
function queryTableEngines(database?: string, table?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
 ...
 * 查询数据库数据总和
 */
function queryDatabaseSummary(database?: string, table?: string) {
 ...
 * 查询数据库表总和
 */
function queryTableSummary(database?: string, table?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
    { value: "lower(name) != 'information_schema'" },
    { key: 'table', value: table },
 ...
 * 查询分离的分区
 */
function queryDetachedPartitions(database?: string, table?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
    { key: 'table', value: table },
  ]);
 ...
 * 查询数据数随时间变化
 */
function queryPartsOverTimeWithRowCount(database?: string, table?: string, startTime?: string, endTime?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
    { key: 'table', value: table },
    {
 ...
 * 查询每个分区最大的部分
 */
function queryMaxPartsPerPartition(database?: string, table?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
    { value: 'active' },
    { key: 'table', value: table },
 ...
 * 查询近期的分析
 */
function queryRecentPartAnalysis(database?: string, table?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
    { key: 'table', value: table },
...
function queryDatabaseNumber (database?: string) {
  const whereStr = dealSQLWhere([
 ...
 */
 ...
 function queryTableNumber (database?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
  ]);
  return new Query(
 ...
 */
 ...
 function queryRowNumber (database?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
  ]);
  return new Query(
 ...
 */
 ...
 function queryColumnNumber (database?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
  ]);
  return new Query(
 ...
 */
 ...
 function queryCurrentMerges (database?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
  ]);
 ...
 */
 ...
 function queryCurrentMutations (database?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
  ]);
  // WHERE database IN (${database:singlequote})
 ...
 */
 ...
 function queryReplicatedTablesByDelay (database?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
  ]);
  // WHERE database IN (${database:singlequote})
 ...
 */
 ...
 function queryMergeProgressPerTable (database?: string) {
  const whereStr = dealSQLWhere([
    { key: 'database', value: database },
  ]);
  // WHERE database IN (${database:singlequote})
 ...
 */
 ...
 function queryMutattionsPartsRemaining (database?: string) {
  const whereStr = dealSQLWhere([
...
function queryAveTimeQueryAnalysis (
  ...
  return new Query(
    // SELECT avg(query_duration_ms) as "Avg query time", $__timeInterval(query_start_time) as time FROM system.query_log WHERE type in ($type) AND initial_user IN ($user) AND query_kind IN ($query_kind) AND $__timeFilter(event_time) GROUP BY time ORDER BY time
    formatJson(
