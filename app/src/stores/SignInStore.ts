...
export default class SignInStore extends ApiRequestableStore {
  @observable
  selectedConnection: ConnectionModel = ConnectionModel.Demo;
  ...
  // async getLastLoginConnect() {
  //   const connectionList = await connectionsStorage.get()
  //   return connectionList.find(item => item.lastLogin)
  // }

  async signIn(history: History) {
    const connectionList = await connectionsStorage.get()
    const { connectionName, connectionUrl } = this.selectedConnection
    const hadNameIndex = connectionList.findIndex(item => item.connectionName === connectionName && item.connectionUrl !== connectionUrl)

    if (hadNameIndex > -1) {
      // throw new Error('The name is already had!')
      // return 'The name is already had!'
      connectionList.splice(hadNameIndex, 1)
      connectionsStorage.saveConnections(connectionList)
    }
