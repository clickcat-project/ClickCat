...
export default class TabsStore extends ApiRequestableStore<DashboardUIStore> {
  ...
  openTableTab(table: ServerStructure.Table) {
    this.openSpecialTab(TabType.TableView, TableViewTabModel.id(table.id), () =>
      TableViewTabModel.from({ tableName: table.name, tableId: table.id, engine: table.engine })
    );
  }
  ...
  async getMutationLists(isOnlySelect: boolean, isCluster: boolean) {
    return this.api.getMutationLists(isOnlySelect, isCluster);
  }
  ...
  async execQueries(query: Query) {
    sqlHistoryStorage.addItems([query.sqlOriginal]);
    runInAction(() => {
      this.uiStore.executingQuery = query;
    });
    const tab = this.getActiveTabOfType<EditorTabModel>(TabType.Editor).get();
    try {
      const fetchResult = await this.api.fetch(query);
      const result = Try.success(fetchResult) ;
      const stats = result.get().stats;
      runInAction(() => {
      ...
      runInAction(() => {
        tab.queriesResult = Some({
          ...
          totalStats: { timeElapsed: 0, rowsRead: 0, bytesRead: 0 },
        });
      });
    }finally {
      runInAction(() => {
        this.uiStore.executingQuery = null!;
      });
    }
  }
