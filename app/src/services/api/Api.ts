import Connection, { ConnectionType, isDemoConnection, isDirectConnection } from '../Connection';
import CoreProvider, {
  QueryResponse,
  QueryResponseKey,
  RequestPool,
...
export default class Api {
  static async connect(connection: Connection): Promise<Api> {
    if (connection.type === ConnectionType.Demo) {
      connection.type = ConnectionType.Direct
    }
    const provider = isDirectConnection(connection) || isDemoConnection(connection)
  ...
  getMutationLists = async (isOnlySelect: boolean, isCluster: boolean): Promise<QueryResponse> =>
    this.provider.getMutationLists(isOnlySelect, isCluster);
